...
(function (root, factory) {
  ...
  return function(data, path) {
     ...
     * the second if is for when nothing is found - if nothing is found, we would return {undefined:undefined}
     * that's not helpful so we set it to an empty object
     */
    if (undefResult) {
      result = result['undefined'];
    } else if (undefResult === undefined && Object.hasOwnProperty(undefined)) {
      result = {};
    }
    return result;
  };
